  ...
  searchChildren(parent, elementType, elementName, includeReferences) {
    ...
    for(let key in parent) {
      ...
      if(this.elementValid(element)) {
        ...
        ret = ret.concat(this.searchChildObject(key, element, elementType, elementName, includeReferences));
      }
    }
    return ret;
  }

  searchChildObject(key, element, elementType, elementName, includeReferences) {
    switch(key) {
      case 'References':
        if(includeReferences) {
          return this.searchChildren(element, elementType, elementName, includeReferences);
        }
        return [];
      case 'Annotations':
